...
public class SlotResolver_OffGCD_自生II : ISlotResolver
{
    Spell spell = new Spell(SageSpell.自生II, SpellTargetType.Self);
    ...
    public int Check_Before_Use()
    {
        ...
        if (!SageRotationEntry.QT.GetQt(QTKey.自生) || !SageRotationEntry.QT.GetQt(QTKey.能力技治疗))
            return -10;
        //检查CD
        if(!SpellExtension.IsReadyWithCanCast(spell))
            return -9;
        //if (SageSpell.自生II.GetSpell().RecentlyUsed(60000))
        //    return -11;
        if (SageSpell.智慧之爱.RecentlyUsed(10000))
            return -2;
        var aoeCount = TargetHelper.GetNearbyEnemyCount(5);
        if (aoeCount >= 2 && SageSpell.自生II.IsReady())
            return 1;
    ...
    {
        slot.Add(SageSpell.自生II.GetSpell());
